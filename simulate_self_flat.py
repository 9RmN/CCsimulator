...
def simulate_self_flat(student_id):
    ...
    success_counter = {dept: 0 for dept in hopes}
    ...
    for _ in range(N_SIMULATIONS):
        ...
        for dept in assigned_depts:
            success_counter[dept] += 1

    result = pd.DataFrame([
        {"希望科": dept, "通過確率（順位無視）": f"{(success_counter[dept] / N_SIMULATIONS * 100):.1f}%"}
        for dept in hopes
    ])
